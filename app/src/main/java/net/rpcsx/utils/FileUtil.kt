...
object FileUtil {
    ...
    fun importConfig(ctx: Context, uri: Uri): Boolean {
        return try {
            ...
            if (docFile == null || (docFile.name?.endsWith(".json", true) != true)) return false
            val inputStream: InputStream = ctx.contentResolver.openInputStream(uri) ?: return false
            val outputFile: File = ctx.getExternalFilesDir(null)?.resolve("config")?.resolve("config.json") ?: return false
    ...
    fun exportConfig(ctx: Context, uri: Uri): Boolean {
        return try {
            val inputFile = ctx.getExternalFilesDir(null)?.resolve("config")?.resolve("config.json") ?: return false
